...
function Sidebar({
  ...
  return (
    <aside
      className={`${collapsed ? 'w-[52px]' : 'w-56'} fixed lg:sticky inset-y-0 left-0 flex-shrink-0 bg-white dark:bg-gray-950 border-r border-gray-200 dark:border-gray-800 flex flex-col transition-all duration-200 h-screen z-50 group/sidebar ${mobileOpen ? 'translate-x-0' : '-translate-x-full'} lg:translate-x-0`}
    >
      {/* Logo + toggle */}
      <div className={`flex items-center ${collapsed ? 'justify-center' : 'gap-3'} px-3 py-3 border-b border-gray-200 dark:border-gray-800 relative`}>
        ...
        {!collapsed && (
          <>
            <div className="overflow-hidden flex-1">
              <div className="text-sm font-bold text-gray-900 dark:text-white leading-tight tracking-[0.12em] font-sans">AAISM</div>
            ...
            <button
              onClick={onToggle}
              className="p-1 rounded-md text-gray-400 dark:text-gray-600 hover:text-gray-700 dark:hover:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors opacity-0 group-hover/sidebar:opacity-100"
        ...
        {collapsed && (
          <button
            onClick={onToggle}
            className="absolute -right-3 top-1/2 -translate-y-1/2 w-6 h-6 rounded-full bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 flex items-center justify-center text-gray-500 dark:text-gray-400 hover:text-gray-900 dark:hover:text-white hover:bg-gray-50 dark:hover:bg-gray-700 transition-all shadow-md opacity-0 group-hover/sidebar:opacity-100 z-50"
      ...
      <nav className="flex-1 overflow-y-auto py-2 px-1.5 space-y-1">
        {navSections.map((section, sIdx) => (
          <div key={section.label}>
            {sIdx > 0 && collapsed && (
              <div className="mx-2 my-2 border-t border-gray-200 dark:border-gray-800" />
            )}
            {!collapsed && (
              <div className="text-[10px] font-semibold text-gray-500 dark:text-gray-500 tracking-widest px-2.5 mb-1 mt-2">
                {section.label}
              </div>
            )}
            <div className="space-y-0.5">
              {section.items.map(item => {
                ...
                return (
                  ...
                  >
                    <Icon className={`w-[18px] h-[18px] flex-shrink-0 ${isActive ? 'text-emerald-700 dark:text-emerald-400' : 'text-gray-500 dark:text-gray-500 group-hover:text-gray-700 dark:group-hover:text-gray-300'}`} />
                    {!collapsed && (
                ...
                <>
                  <button
                    ...
                    className={`w-full flex items-center gap-3 px-2.5 py-2 rounded-lg text-sm font-medium transition-all ${
                      isSupportMoreActive ? 'text-emerald-700 dark:text-emerald-400' : 'text-gray-500 hover:text-gray-700 dark:hover:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-800'
                    }`}
                  >
                  ...
                  {(supportExpanded || isSupportMoreActive) && section.moreItems.map(item => {
                    ...
                    return (
                      ...
                      >
                        <Icon className={`w-4 h-4 flex-shrink-0 ${isActive ? 'text-emerald-700 dark:text-emerald-400' : 'text-gray-400 dark:text-gray-600 group-hover:text-gray-600 dark:group-hover:text-gray-300'}`} />
                        <span className="truncate">{item.label}</span>
                      </NavLink>
                    );
                  })}
      ...
      {/* User / Level */}
      <div className={`border-t border-gray-200 dark:border-gray-800 p-2 ${collapsed ? 'flex justify-center' : ''}`}>
        <div className={`flex items-center ${collapsed ? 'justify-center' : 'gap-3 px-1'}`}>
          ...
          {!collapsed && (
            <div className="overflow-hidden flex-1">
              <div className="text-xs font-medium text-gray-700 dark:text-gray-300 truncate">{currentLevel.title}</div>
    ...
    <header className="h-12 flex items-center justify-between px-4 bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700 flex-shrink-0">
      ...
      <div className="flex items-center gap-2">
        <button
          ...
          className="hidden sm:flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg text-xs text-gray-600 dark:text-gray-400 bg-gray-100 dark:bg-gray-700 hover:text-gray-800 dark:hover:text-gray-300 transition-colors"
          title="Search (⌘K)"
        >
          <span>Search</span>
